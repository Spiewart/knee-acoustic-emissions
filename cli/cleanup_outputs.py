...
⚠️  DATABASE-AWARE CLEANUP:
- Processing records persisted to PostgreSQL are NEVER deleted
- Excel logs are regeneratable from database records (safe to delete)
- Running cleanup + reprocessing = fresh Excel logs from same DB records
- This preserves the audit trail of all processing
...
Does NOT remove:
- Database records in PostgreSQL (preserved for audit trail)
...
Usage:
    ...
    python -m cli.cleanup_outputs /path/to/studies --limit 5
"""
