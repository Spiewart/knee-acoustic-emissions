...
has_periodic_noise = check_cycle_periodic_noise(
    cycle_audio_data, fs=sampling_rate, threshold=0.3
)
...
def check_cycle_periodic_noise(
    ...
    if not available_channels or time_col not in cycle_df.columns:
        return dict.fromkeys(available_channels, False)
...
def validate_acoustic_waveform(
    ...
    if reference_waveform is not None:
        ...
        return _validate_against_reference_waveform(
            rms_envelope, reference_waveform, correlation_threshold, dtw_threshold
        )
    else:
    ...
    if dtw_threshold is not None:
        try:
            ...
            if dtw_distance > dtw_threshold:
                return (
                    False,
                    f"high DTW distance (d={dtw_distance:.3f} > {dtw_threshold:.3f}), correlation={correlation:.3f}",
                )
...
def run_cycle_audio_qc(
    ...
    if check_periodic_noise:
        results["periodic_noise"] = check_cycle_periodic_noise(cycle_df, time_col, audio_channels)
...
def detect_periodic_artifacts_sync_level(
    ...
    *,
    ...
    movement_freq = _estimate_movement_frequency(exercise_df, knee_angle_col, fs)
    ...
    if ds_factor > 1:
        ...
        fs_ds = fs / ds_factor
        logger.debug(
            "Periodic detection: downsampling %dx (%.0f Hz → %.0f Hz, %d → ~%d samples)",
            ds_factor,
            fs,
            fs_ds,
            len(exercise_time),
            len(exercise_time) // ds_factor,
        )
    else:
        fs_ds = fs
    ...
    for ch in available_channels:
        ...
        if ds_factor > 1:
            ...
            ch_time = np.linspace(exercise_time[0], exercise_time[-1], len(ch_data))
        else:
...
def detect_cycle_intermittent_artifacts(
    ...
    *,
    ...
    from src.audio.raw_qc import _mask_to_intervals, _sliding_window_rms
...
def detect_cycle_artifacts(
    ...
    *,
    ...
    if use_ml:
        try:
            intermittent_per_ch = classify_cycle_artifacts_ml(cycle_df, maneuver, time_col, audio_channels)
        except NotImplementedError:
            intermittent_per_ch = detect_cycle_intermittent_artifacts(cycle_df, time_col, audio_channels)
    else:
        intermittent_per_ch = detect_cycle_intermittent_artifacts(cycle_df, time_col, audio_channels)
...
def validate_sync_quality_rule_based(
    ...
    waveform_valid, waveform_reason = validate_acoustic_waveform(cycle_df, maneuver, time_col, audio_channels)
    ...
    if knee_angle_col not in cycle_df.columns or time_col not in cycle_df.columns:
        return waveform_valid, 0.5, "waveform ok, no biomechanics cross-ref available"

    knee_angle = pd.to_numeric(cycle_df[knee_angle_col], errors="coerce").values
    valid_mask = ~np.isnan(knee_angle)
    if valid_mask.sum() < 10:
        return waveform_valid, 0.5, "waveform ok, insufficient biomechanics data"
    ...
    if maneuver == "walk":
        ...
        return is_valid, proximity_score, (f"walk cross-ref: proximity={proximity_score:.2f}")
        ...
        transition_ratio = float(energy_thirds[1]) / max(float(energy_thirds[0]), float(energy_thirds[2]), 1e-10)
        score = min(transition_ratio / 2.0, 1.0)  # Normalize
        is_valid = transition_ratio >= 1.1
        return is_valid, score, (f"STS cross-ref: transition_ratio={transition_ratio:.2f}")
        ...
        mid_range_peaks = [
            p for p in acoustic_peaks if valid_mask[p] and abs(knee_angle[p] - mid_angle) < mid_tolerance
        ]
...
def validate_sync_quality(
    ...
    if use_model:
        try:
            return validate_sync_quality_model_based(cycle_df, maneuver, time_col, audio_channels, knee_angle_col)
        except NotImplementedError:
            pass

    return validate_sync_quality_rule_based(cycle_df, maneuver, time_col, audio_channels, knee_angle_col)
