...
class ReportGenerator:
    ...
    def _coerce_excel_compatible(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if isinstance(series.dtype, pd.DatetimeTZDtype):
                ...
                df[col] = series.apply(
                    lambda v: v.replace(tzinfo=None) if isinstance(v, datetime) and v.tzinfo is not None else v
                )
        return df

    @staticmethod
    def _format_list_for_excel(value) -> str | None:
    ...
    def generate_audio_sheet(self, study_id: int, maneuver: str, knee: str) -> pd.DataFrame:
        ...
        for record in records:
            ...
            if record.biomechanics_import_id:
                ...
                biomech_record = (
                    self.session.query(BiomechanicsImportRecord)
                    .filter(BiomechanicsImportRecord.id == record.biomechanics_import_id)
                    .first()
                )
            ...
            recording_datetime = (
                recording_dt_parsed or record.recording_date or record.recording_time or record.file_time
            )
            ...
            data.append(
                {
                    ...
                    "Biomechanics Sample Rate (Hz)": biomech_record.biomechanics_sample_rate
                    if biomech_record
                    else None,
    ...
    def generate_biomechanics_sheet(self, study_id: int, maneuver: str, knee: str) -> pd.DataFrame:
    ...
    def generate_synchronization_sheet(self, study_id: int, maneuver: str, knee: str) -> pd.DataFrame:
    ...
    def generate_movement_cycles_sheet(self, study_id: int, maneuver: str, knee: str) -> pd.DataFrame:
        ...
        query = (
            self.session.query(MovementCycleRecord)
            .join(AudioProcessingRecord, MovementCycleRecord.audio_processing_id == AudioProcessingRecord.id)
            .outerjoin(SynchronizationRecord, MovementCycleRecord.synchronization_id == SynchronizationRecord.id)
            .filter(
                MovementCycleRecord.study_id == study_id,
                AudioProcessingRecord.maneuver == maneuver,
                AudioProcessingRecord.knee == knee,
                MovementCycleRecord.is_active == True,
            )
            .order_by(func.coalesce(SynchronizationRecord.pass_number, 0), MovementCycleRecord.cycle_index)
        )
            ...
            data.append(
                {
                    ...
                    "Audio Artifact Dropout Timestamps": self._format_list_for_excel(
                        record.audio_artifact_dropout_timestamps
                    ),
                    "Audio Artifact Dropout Timestamps Ch1": self._format_list_for_excel(
                        record.audio_artifact_dropout_timestamps_ch1
                    ),
                    "Audio Artifact Dropout Timestamps Ch2": self._format_list_for_excel(
                        record.audio_artifact_dropout_timestamps_ch2
                    ),
                    "Audio Artifact Dropout Timestamps Ch3": self._format_list_for_excel(
                        record.audio_artifact_dropout_timestamps_ch3
                    ),
                    "Audio Artifact Dropout Timestamps Ch4": self._format_list_for_excel(
                        record.audio_artifact_dropout_timestamps_ch4
                    ),
                    ...
                    "Audio Artifact Continuous Timestamps": self._format_list_for_excel(
                        record.audio_artifact_continuous_timestamps
                    ),
                    "Audio Artifact Continuous Timestamps Ch1": self._format_list_for_excel(
                        record.audio_artifact_continuous_timestamps_ch1
                    ),
                    "Audio Artifact Continuous Timestamps Ch2": self._format_list_for_excel(
                        record.audio_artifact_continuous_timestamps_ch2
                    ),
                    "Audio Artifact Continuous Timestamps Ch3": self._format_list_for_excel(
                        record.audio_artifact_continuous_timestamps_ch3
                    ),
                    "Audio Artifact Continuous Timestamps Ch4": self._format_list_for_excel(
                        record.audio_artifact_continuous_timestamps_ch4
                    ),
                    ...
                    "Audio Artifact Periodic Timestamps": self._format_list_for_excel(
                        record.audio_artifact_periodic_timestamps
                    ),
                    "Audio Artifact Periodic Timestamps Ch1": self._format_list_for_excel(
                        record.audio_artifact_periodic_timestamps_ch1
                    ),
                    "Audio Artifact Periodic Timestamps Ch2": self._format_list_for_excel(
                        record.audio_artifact_periodic_timestamps_ch2
                    ),
                    "Audio Artifact Periodic Timestamps Ch3": self._format_list_for_excel(
                        record.audio_artifact_periodic_timestamps_ch3
                    ),
                    "Audio Artifact Periodic Timestamps Ch4": self._format_list_for_excel(
                        record.audio_artifact_periodic_timestamps_ch4
                    ),
    ...
    def generate_summary_sheet(self, study_id: int, maneuver: str, knee: str) -> pd.DataFrame:
        ...
        audio_count = (
            self.session.query(AudioProcessingRecord)
            .filter(
                AudioProcessingRecord.study_id == study_id,
                AudioProcessingRecord.maneuver == maneuver,
                AudioProcessingRecord.knee == knee,
        ...
        sync_count = (
            self.session.query(func.count(func.distinct(SynchronizationRecord.sync_file_name)))
            .join(
                AudioProcessingRecord,
                SynchronizationRecord.audio_processing_id == AudioProcessingRecord.id,
            )
        ...
        study_record = self.session.query(StudyRecord).filter(StudyRecord.id == study_id).first()
    ...
    def save_to_excel(
        self, output_path: Path, participant_id: int, maneuver: str, knee: str, legend_mismatches: list | None = None
    ) -> Path:
        ...
        if legend_mismatches:
            sheets["Legend Validation"] = self.generate_legend_validation_sheet(legend_mismatches)
