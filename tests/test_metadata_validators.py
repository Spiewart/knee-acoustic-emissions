...
class TestSynchronizationMetadataValidators:
    def test_sync_method_requirements(self):
        base_kwargs = dict(
            ...
            aligned_bio_sync_time=timedelta(seconds=8.5),
            bio_left_sync_time=timedelta(seconds=8.5),
            bio_right_sync_time=None,
            consensus_time=timedelta(seconds=10.0),
            rms_time=timedelta(seconds=10.0),
            onset_time=timedelta(seconds=10.0),
            freq_time=timedelta(seconds=10.0),
        )
    ...
    def test_to_dict_converts_timedelta_to_seconds(self):
        ...
        d = sync_meta.to_dict()
        assert d["Audio Sync Time"] == 10.0
        assert d["Sync Offset"] == 1.5
