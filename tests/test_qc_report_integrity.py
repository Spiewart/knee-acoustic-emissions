...
def _create_mock_record(
    ...
    record.qc_artifact = qc_artifact
    record.qc_artifact_type = qc_artifact_type
    record.qc_artifact_ch1 = qc_artifact_ch1
    record.qc_artifact_type_ch1 = qc_artifact_type_ch1
    record.qc_artifact_ch2 = qc_artifact_ch2
    record.qc_artifact_type_ch2 = qc_artifact_type_ch2
    record.qc_artifact_ch3 = qc_artifact_ch3
    record.qc_artifact_type_ch3 = qc_artifact_type_ch3
    record.qc_artifact_ch4 = qc_artifact_ch4
    record.qc_artifact_type_ch4 = qc_artifact_type_ch4
...
class TestQCFailColumnComputation:
    ...
    def test_qc_fail_true_when_dropout(self):
        ...
        record = _create_mock_record(qc_signal_dropout=True, qc_artifact=False)

        # QC Fail should be computed as dropout OR artifact
        qc_fail = record.qc_signal_dropout or record.qc_artifact
...
class TestArtifactTypePopulation:
    ...
    def test_artifact_type_blank_when_no_artifacts(self):
        ...
        assert record.qc_artifact_type is None or record.qc_artifact_type == [], \
    ...
    def test_artifact_type_populated_when_artifacts(self):
        ...
        artifact_types = record.qc_artifact_type
    ...
    def test_per_mic_artifact_type_matches_detection(self):
        ...
        assert record.qc_artifact_ch1 is True
        assert record.qc_artifact_type_ch1 is not None

        # Ch2: no artifact, should have no type
        assert record.qc_artifact_ch2 is False
        assert record.qc_artifact_type_ch2 is None

        # Ch3: has artifact, should have type
        assert record.qc_artifact_ch3 is True
        assert record.qc_artifact_type_ch3 is not None

        # Ch4: no artifact, should have no type
        assert record.qc_artifact_ch4 is False
        assert record.qc_artifact_type_ch4 is None
    ...
    def test_mixed_clean_and_failing_channels(self):
        ...
        record = _create_mock_record(
            qc_signal_dropout=False,
            qc_artifact=True,  # Overall artifact detected
    ...
    def test_dropout_without_artifacts(self):
        ...
        assert record.qc_artifact is False
        assert record.qc_artifact_type is None


class TestArtifactTypeEnforcement:
    """Test suite for artifact type enforcement rules.

    These tests verify that when qc_artifact=True, artifact type is NEVER blank.
    """
    ...
    def test_intermittent_artifact_classification(self):
        ...
        assert record.qc_artifact is True
        assert record.qc_artifact_type is not None
        assert "Intermittent" in record.qc_artifact_type
    ...
    def test_continuous_artifact_classification(self):
        ...
        assert record.qc_artifact is True
        assert record.qc_artifact_type is not None
        assert "Continuous" in record.qc_artifact_type
    ...
    def test_multiple_artifact_types(self):
        ...
        assert record.qc_artifact is True
        assert len(record.qc_artifact_type) == 2
        assert all(t in ["Intermittent", "Continuous"] for t in record.qc_artifact_type)
    ...
    def test_artifact_type_required_per_channel(self):
        ...
        if record.qc_artifact_ch1:
            assert record.qc_artifact_type_ch1 is not None, \
                "Ch1 has artifact but type is None/blank"

        if record.qc_artifact_ch2:
            assert record.qc_artifact_type_ch2 is not None, \
                "Ch2 has artifact but type is None/blank"

        # Ch3 and Ch4 should not have types since they don't have artifacts
        if not record.qc_artifact_ch3:
            assert record.qc_artifact_type_ch3 is None or len(record.qc_artifact_type_ch3) == 0, \
                "Ch3 has no artifact but type is populated"

        if not record.qc_artifact_ch4:
            assert record.qc_artifact_type_ch4 is None or len(record.qc_artifact_type_ch4) == 0, \
