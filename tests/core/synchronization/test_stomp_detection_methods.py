...
def create_synthetic_stomp_audio(sr=10000, duration=3.0, stomp_time=1.5):
    ...
    int(stomp_time * sr)
    int(0.15 * sr)  # 150ms stomp duration
...
def test_rms_energy_stomp_detection():
    ...
    assert abs(rms_time - 1.5) < 0.2, (
        f"RMS method should detect stomp within 200ms, got error of {abs(rms_time - 1.5):.3f}s"
    )
...
def test_impact_onset_stomp_detection():
    ...
    assert abs(onset_time - 1.5) < 0.2, (
        f"Onset method should detect stomp within 200ms, got error of {abs(onset_time - 1.5):.3f}s"
    )
...
def test_frequency_content_stomp_detection():
    ...
    assert abs(freq_time - 1.5) < 0.2, (
        f"Frequency method should detect stomp within 200ms, got error of {abs(freq_time - 1.5):.3f}s"
    )
...
def test_consensus_stomp_detection():
    ...
    assert abs(consensus - 1.5) < 0.2, (
        f"Consensus should detect stomp within 200ms, got error of {abs(consensus - 1.5):.3f}s"
    )
...
def test_full_stomp_detection_without_biomechanics():
    ...
    assert abs(detected_stomp_s - 2.3) < 0.2, (
        f"Stomp should be detected within 200ms of expected time, got error of {abs(detected_stomp_s - 2.3):.3f}s"
    )
...
def test_stomp_detection_with_left_knee_biomechanics():
    ...
    assert abs(detected_left.total_seconds() - 1.0) < 0.5, (
        f"Left knee stomp should be detected near first stomp (1.0s), got {detected_left.total_seconds():.3f}s"
    )
...
def test_stomp_detection_with_right_knee_biomechanics():
    ...
    assert abs(detected_right.total_seconds() - 3.0) < 0.5, (
        f"Right knee stomp should be detected near second stomp (3.0s), got {detected_right.total_seconds():.3f}s"
    )
