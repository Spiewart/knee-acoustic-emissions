...
def test_biomech_guided_detection_right_then_left():
    ...
    right_time = 5.0
    left_time = 8.0
    left_time - right_time  # 3.0 seconds
    ...
    assert abs(detected_seconds - right_time) < 0.3, f"Expected ~{right_time}s, got {detected_seconds}s"
...
def test_biomech_guided_detection_left_then_right():
    ...
    left_time = 4.0
    right_time = 7.0
    right_time - left_time  # 3.0 seconds
    ...
    assert abs(detected_seconds - left_time) < 0.3, f"Expected ~{left_time}s, got {detected_seconds}s"
...
def test_biomech_guided_detection_close_stomps():
    ...
    assert abs(detected_seconds - stomp1_time) < 0.3, f"Expected ~{stomp1_time}s, got {detected_seconds}s"
...
def test_biomech_guided_detection_far_apart_stomps():
    ...
    assert abs(detected_seconds - stomp1_time) < 0.5, f"Expected ~{stomp1_time}s, got {detected_seconds}s"
...
def test_biomech_guided_detection_with_extra_peaks():
    ...
    assert abs(detected_seconds - right_time) < 0.4, f"Expected ~{right_time}s, got {detected_seconds}s"
...
def test_biomech_guided_fallback_to_consensus():
    ...
    assert abs(detected_seconds - consensus_time) < 0.1, "Should have fallen back to consensus time"
...
def test_biomech_guided_energy_prioritization():
    ...
    assert abs(detected_seconds - 5.0) < 0.4, f"Expected high-energy peak at 5.0s, got {detected_seconds}s"
...
def test_backward_compatibility_without_biomechanics():
    ...
    assert abs(detected_seconds - stomp_time) < 0.5, f"Expected ~{stomp_time}s, got {detected_seconds}s"
