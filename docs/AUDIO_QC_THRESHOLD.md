# Audio QC Threshold Configuration

## Overview

All signal dropout detection functions in `src/audio/raw_qc.py` now use **centralized threshold configuration** through the `DEFAULT_DROPOUT_THRESHOLDS` dataclass. See [QC_THRESHOLD_CALIBRATION_FIX](../changelog/bugfix/QC_THRESHOLD_CALIBRATION_FIX.md) in the changelog for technical details on how these thresholds were calibrated.
